...
export default function CardsGrid({ content }: PublicContentProps) {
  const characterCards = content.cards.filter((card) => card.section === "character");
  const cardsToRender = characterCards.length > 0 ? characterCards : content.cards;
    ...
    <section id="movies" className="mx-auto mt-8 max-w-6xl px-4 pb-12 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-4 gap-3 sm:gap-4">
        {cardsToRender.map((card) => (
          ...
          >
            <article className="overflow-hidden rounded-md border border-stone-300 bg-white shadow-sm">
